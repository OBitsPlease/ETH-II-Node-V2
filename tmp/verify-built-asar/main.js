...
const SEND_RPC_CANDIDATES = [PRIMARY_RPC_URL, SECONDARY_RPC_URL, PUBLIC_RPC_URL];
...
ipcMain.handle('send-tx', async (_, { privateKey, to, amount, gasPrice }) => {
  try {
    ...
    const p = await resolveWorkingProvider(SEND_RPC_CANDIDATES, false);
...
ipcMain.handle('get-tx-history', async (_, { address, limit = 200, scan = 3000 }) => {
  try {
    if (!address) throw new Error('No address provided');

    const needle = String(address).toLowerCase();
    const maxItems = Math.min(Math.max(parseInt(limit, 10) || 200, 1), 500);
    const maxScan = Math.min(Math.max(parseInt(scan, 10) || 3000, 100), 20000);
