...
const SEND_RPC_CANDIDATES = [PRIMARY_RPC_URL, SECONDARY_RPC_URL];
...
async function resolveWorkingProvider(candidates = READ_RPC_CANDIDATES) {
  const urls = [...new Set([...(Array.isArray(candidates) ? candidates : []), RPC_URL])];
  let lastErr;
...
ipcMain.handle('send-tx', async (_, { privateKey, to, amount, gasPrice }) => {
  try {
    ...
    const p = await resolveWorkingProvider(SEND_RPC_CANDIDATES);
