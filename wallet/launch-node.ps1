...
if (Test-Path $KEY) {
  $tunnelProc = Get-Process -Name ssh -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -like '*-L*8545*' } | Select-Object -First 1
  if (-not $tunnelProc) {
    Write-Host "Starting SSH RPC tunnel to primary VPS (localhost:8545 -> 87.99.142.128:8545)..." -ForegroundColor Cyan
    Start-Process ssh -ArgumentList "-i `"$KEY`" -N -L 8545:127.0.0.1:8545 root@87.99.142.128" -WindowStyle Hidden | Out-Null
