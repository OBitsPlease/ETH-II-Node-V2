...
using the pool's ports (30303, 3335, 3334, 3336, 8082, 8545), it aborts
...
## 3. Important: Your pool wallet is locked at install time

The setup automatically generates a **pool wallet address** that is used to collect all block rewards. This address **cannot be changed** after installation without reinstalling.

- **Your pool wallet** (generated by setup): receives all block mining rewards
- **Miner wallets** (chosen by each miner): each miner's address becomes their username when connecting to your pool. They receive payouts from your pool wallet based on their shares.

**Find your pool wallet address:**

After setup completes, your pool address is displayed and saved in:
- `/opt/ethii/POOL-INFO.txt` — full wallet and connection details
- `/opt/ethii/MINERS-CONNECT-HERE.txt` — what to tell your miners
