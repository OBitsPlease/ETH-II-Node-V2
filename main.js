...
async function resolveWorkingProvider(candidates = READ_RPC_CANDIDATES, includeLocalFallback = false) {
  const urls = [...new Set([
    ...(Array.isArray(candidates) ? candidates : []),
    ...(includeLocalFallback ? [RPC_URL] : []),
  ])];
  let lastErr;
...
async function resolveChainIdForSigning() {
  try {
    const chainIdHex = await rpcCallRead('eth_chainId', []);
    if (typeof chainIdHex === 'string' && chainIdHex.startsWith('0x')) {
      const parsed = Number.parseInt(chainIdHex, 16);
      if (Number.isFinite(parsed) && parsed > 0) return parsed;
    }
    const parsed = Number.parseInt(String(chainIdHex), 10);
    if (Number.isFinite(parsed) && parsed > 0) return parsed;
  } catch (e) {
    // Fall back if RPC chain-id lookup is temporarily unavailable.
  }
...
ipcMain.handle('send-tx', async (_, { privateKey, to, amount, gasPrice }) => {
  try {
    ...
    if (!p) throw new Error('Canonical send RPC unavailable. Start wallet via launch-node.ps1 so localhost tunnel is active.');
    ...
    const nonceHex = await p.send('eth_getTransactionCount', [from, 'pending']);
    const nonce = typeof nonceHex === 'string' && nonceHex.startsWith('0x')
      ? Number.parseInt(nonceHex, 16)
      : Number.parseInt(String(nonceHex), 10);
    ...
    const txHash = await p.send('eth_sendRawTransaction', [signed]);
...
// Get transaction history for an address by scanning recent chain blocks in parallel batches.
ipcMain.handle('get-tx-history', async (_, { address, limit = 300, scan = 20000 }) => {
  try {
    if (!address) throw new Error('No address provided');

    const needle = String(address).toLowerCase();
    const pendingNow = readPendingTxs()
      .filter((p) => {
      ...
      .map((p) => ({
        ...
        direction: String((p.to || '')).toLowerCase() === needle ? 'in' : 'out',
      }));
    if (pendingNow.length > 0) {
      pendingNow.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
      return { success: true, txs: pendingNow, scannedBlocks: 0, latestBlock: null, pendingOnly: true };
    }

    const maxItems = Math.min(Math.max(parseInt(limit, 10) || 300, 1), 800);
    const maxScan = Math.min(Math.max(parseInt(scan, 10) || 3000, 500), 12000);
